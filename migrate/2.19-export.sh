...
# Constants
export MONGO_DATA="/data/circle/mongo"
export MONGO_VERSION="3.6.6"
export TMP_MONGO="circle-mongo-export"
export PGDATA="/data/circle/postgres/9.5/data"
export PGVERSION="9.5.8"
export TMP_PSQL="circle-postgres-export"
export KEY_BU="circle-data"
...
function circleci_database_export() {
    ...
    if [ ! "$SKIP_MONGO" = "true" ];
    then
        export MONGO_BU="circleci-mongo-export"
        start_mongo
        export_mongo
        check_mongo
        stop_mongo
    ...
    then
        export PG_BU="circleci-pg-export"
        start_postgres
        export_postgres
        check_postgres
        stop_postgres
    ...
    if [ ! "$SKIP_VAULT" = "true" ];
    then
        export VAULT_BU="circleci-vault"
        export_vault
    fi
